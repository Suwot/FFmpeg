...
static int mov_read_saiz(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (encryption_index->auxiliary_info_default_size == 0) {
        if (sample_count == 0) {
            if (!sc->cenc.per_sample_iv_size) {
                av_log(c->fc, AV_LOG_DEBUG,
                       "Ignoring empty saiz box for constant-IV full-sample encryption\n");
                return 0;
            }
            return AVERROR_INVALIDDATA;
        }
...
static const AVOption mov_options[] = {
    ...
    { "ignore_hier_sidx", "Ignore hierarchical SIDX (reference_type=1): warn and keep timing, but do not advance byte offsets.", OFFSET(ignore_hier_sidx), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, AV_OPT_FLAG_DECODING_PARAM },
